...
class EngineTests(unittest.TestCase):
    ...
    def get_engine(
        ...
        for node_name, ffs_to_snapshots in quick_ffs_definition.items():
            ...
            e.incoming_node({"msg": "ffs_list", "from": node_name, "ffs": ffs})
    ...
    def ge(self, config=None):
        ...
        config._nodes = nodes
        config = default_config.CheckedConfig(config)
        return engine.Engine(config, fm), fm.outgoing
...
class StartupTests(EngineTests):
    def test_nodes_may_not_start_with_dash(self):
        ...
        def inner():
            ...
            cfg._nodes = nodes
            engine.Engine(default_config.CheckedConfig(cfg), ignore)
    ...
    def test_startup_sends_deployment_followed_by_list_ffs(self):
        ...
        self.assertMsgEqual(
            strip_node_zip(outgoing_messages[0]),
            {"to": "alpha", "msg": "deploy", "node.zip": "...base64..."},
        )
        self.assertMsgEqual(
            strip_node_zip(outgoing_messages[1]),
            {"to": "beta", "msg": "deploy", "node.zip": "...base64..."},
        )
        outgoing_messages.clear()
        e.incoming_node({"msg": "deploy_done", "from": "alpha"})
        e.incoming_node({"msg": "deploy_done", "from": "beta"})
        self.assertFalse(e.startup_done)
        self.assertMsgEqual(outgoing_messages[0], {"to": "alpha", "msg": "list_ffs"})
        self.assertMsgEqual(outgoing_messages[1], {"to": "beta", "msg": "list_ffs"})
        outgoing_messages.clear()

        e.incoming_node({"msg": "ffs_list", "from": "beta", "ffs": {}})
    ...
    def test_list_ffs_failure_leads_to_faulted_engine(self):
        ...
        def inner():
            e.incoming_node(
                {
                    "error": "exception",
                    "content": "storage prefix not a ZFS",
    ...
    def test_restoring_main_from_ro(self):
        ...
        e.incoming_node(
            {
                "msg": "ffs_list",
                "from": "beta",
                "ffs": {"one": {"snapshots": [], "properties": {"readonly": "off"}}},
            }
        )
        e.incoming_node({"msg": "ffs_list", "from": "alpha", "ffs": {}})
        self.assertTrue(e.startup_done)
        ...
        self.assertEqual(e.model["one"]["beta"]["properties"]["readonly"], "off")
        # no ffs info -> nothing happens
        ...
        self.assertEqual(e.model["one"]["beta"]["properties"]["readonly"], "off")
        self.assertEqual(e.model["one"]["beta"]["properties"]["ffs:main"], "on")
        ...
        e.incoming_node(
            {
                "msg": "ffs_list",
                "from": "alpha",
                "ffs": {"one": {"snapshots": [], "properties": {"readonly": "on"}}},
            }
        )
        self.assertMsgEqual(
            outgoing_messages[0],
        ...
        e.incoming_node(
            {
                "msg": "ffs_list",
                "from": "beta",
                "ffs": {"one": {"snapshots": [], "properties": {"ffs:main": "off"}}},
            }
        )
        ...
        e.incoming_node(
            {
                ...
                "ffs": {
                    "one": {
                        "snapshots": [],
                        "properties": {"ffs:main": "on", "readonly": "on"},
                    }
                },
            }
        )
            ...
            e.incoming_node(
                {
                    ...
                    "from": "alpha",
                    "ffs": {
                        "one": {"snapshots": [], "properties": {"readonly": "off"}}
                    },
                }
            )
    ...
    def test_no_main_but_remove_asap_on_all(self):
        ...
        self.assertMsgEqual(
            outgoing_messages[0], {"msg": "remove", "to": "alpha", "ffs": "one"}
        )
        self.assertMsgEqual(
            outgoing_messages[1], {"msg": "remove", "to": "beta", "ffs": "one"}
        )
        ...
        e.incoming_node(
            {
                "msg": "ffs_list",
                "from": "beta",
                "ffs": {"one": {"snapshots": [], "properties": {"ffs:main": "on"}}},
            }
        )
        ...
        e.incoming_node(
            {
                ...
                "from": "beta",
                "ffs": {
                    "one": {"snapshots": ["ffs-1"], "properties": {"ffs:main": "off"}}
                },
            }
        )
    ...
    def test_prune_on_startup_leaves_at_least_one_snapshot(self):
        ...
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1", "2", "3"]}, "beta": {"one": ["1", "2", "3"]}},
            config=cfg,
        )
        self.assertEqual(4, len(outgoing_messages))
        self.assertMsgEqual(
            outgoing_messages[0],
            {"msg": "remove_snapshot", "ffs": "one", "snapshot": "1", "to": "alpha"},
        )
        self.assertMsgEqual(
            outgoing_messages[1],
            {"msg": "remove_snapshot", "ffs": "one", "snapshot": "2", "to": "alpha"},
        )
        self.assertMsgEqual(
            outgoing_messages[2],
            {"msg": "remove_snapshot", "ffs": "one", "snapshot": "1", "to": "beta"},
        )
        self.assertMsgEqual(
            outgoing_messages[3],
            {"msg": "remove_snapshot", "ffs": "one", "snapshot": "2", "to": "beta"},
        )
...
class PostStartupTests(EngineTests):
    def ge(self, cfg=None):
        ...
        if not hasattr(cfg, "_nodes"):
            cfg._nodes = nodes
        fm = FakeMessageSender()
        e = engine.Engine(default_config.CheckedConfig(cfg), fm)
        e.incoming_client({"msg": "startup"})
        e.incoming_node(
            {
                "msg": "ffs_list",
...
class NewTests(PostStartupTests):
    def test_new_raises_on_existing(self):
        ...
        def inner():
            e.incoming_client({"msg": "new", "ffs": "one", "targets": ["alpha"]})
    ...
    def test_non_list_targets_raises(self):
        ...
        def inner():
            e.incoming_client({"msg": "new", "ffs": "two", "targets": "gamma"})
    ...
    def test_new_raises_on_non_existing_targets(self):
        ...
        def inner():
            e.incoming_client({"msg": "new", "ffs": "one", "targets": ["gamma"]})
    ...
    def test_new_multi_nodes(self):
        e, outgoing_messages = self.ge()
        e.incoming_client({"msg": "new", "ffs": "two", "targets": ["beta", "alpha"]})
        self.assertEqual(len(outgoing_messages), 2)
        self.assertMsgEqual(
            outgoing_messages[1],
            {
    ...
    def test_name_ok_callback(self):
        e, outgoing_messages = self.ge()
        e.incoming_client({"msg": "new", "ffs": "Three", "targets": ["beta", "alpha"]})
        ...
        def inner():
            e.incoming_client(
                {"msg": "new", "ffs": "Three", "targets": ["beta", "alpha"]}
            )
    ...
    def test_remove_while_new_unrelated(self):
        ...
        outgoing_messages.clear()
        e.incoming_client(
            {"msg": "new", "ffs": "two", "targets": ["beta", "alpha", "gamma"]}
        )
    ...
    def test_new_sets_default_properties(self):
        ...
        cfg.get_default_properties = lambda: {
            "ffs:test": 23
        }  # also tests stringification
        e, outgoing_messages = self.ge(cfg)
        e.incoming_client({"msg": "new", "ffs": "two", "targets": ["alpha"]})
        self.assertEqual(1, len(outgoing_messages))
        self.assertEqual(
            outgoing_messages[0]["properties"],
            {"ffs:main": "on", "readonly": "off", "ffs:test": "23"},
        )

    def test_new_if_parent_is_not_on_target_fails(self):
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1"], "_one/two": ["1"]}, "beta": {}}
        )
    ...
    def test_nested_creation_delayed_until_parent_has_been_created(self):
        ...
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1"], "_two": ["1"]}, "beta": {}}, sender_cls=om
        )
        e.incoming_client({"msg": "new", "ffs": "one/a", "targets": ["alpha"]})
        self.assertEqual(
            len([x for x in outgoing_messages["alpha"] if x.status == "in_progress"]), 1
        )
        e.incoming_client({"msg": "new", "ffs": "two/b", "targets": ["alpha"]})
        self.assertEqual(
            len([x for x in outgoing_messages["alpha"] if x.status == "in_progress"]), 2
        )
        e.incoming_client({"msg": "new", "ffs": "three", "targets": ["alpha"]})
        self.assertEqual(
            len([x for x in outgoing_messages["alpha"] if x.status == "in_progress"]), 3
        )
    ...
    def test_sibling_new_delays(self):
        # make sure that shu/one and shu/two get run seperatly (read only
        # issues!)
        cfg = self._get_test_config()
        cfg.get_ssh_concurrent_connection_limit = lambda: 5
        omtf = OutgoingMessageForTesting()
    ...
    def test_new_parent_issue(self):
        ...
        e, outgoing_messages = self.get_engine(
            {
                "alpha": {
                    "_papers": ["1"],
                    "_papers/mueller": ["1"],
                    "_papers/mueller/20141031": ["1"],
                    "_papers/adhikary": ["1"],
                    "_papers/adhikary/shu": ["1"],
                },
        ...
        self.assertRaises(ValueError, inner)
        e.incoming_client(
            {"msg": "add_targets", "ffs": "papers/adhikary/shu", "targets": ["beta"]}
        )

        self.assertEqual(
            len([x for x in outgoing_messages["alpha"] if x.status == "in_progress"]), 0
        )
        self.assertEqual(
            len([x for x in outgoing_messages["beta"] if x.status == "in_progress"]), 1
        )

        e.incoming_client(
            {"msg": "add_targets", "ffs": "papers/mueller", "targets": ["beta"]}
        )
        self.assertEqual(
            len([x for x in outgoing_messages["alpha"] if x.status == "in_progress"]), 0
        )
        self.assertEqual(
            len([x for x in outgoing_messages["beta"] if x.status == "in_progress"]), 2
        )
        e.incoming_client(
            {
                "msg": "add_targets",
    ...
    def test_new_parent_issue(self):
        ...
        omtf.outgoing["alpha"] = []
        omtf.outgoing["beta"] = []
        e.incoming_client({"msg": "add_targets", "ffs": "papers", "targets": ["beta"]})
        self.assertEqual(
            len([x for x in outgoing_messages["alpha"] if x.status == "in_progress"]), 0
        )
        self.assertEqual(
        ...
        e.incoming_client(
            {"msg": "add_targets", "ffs": "papers/adhikary", "targets": ["beta"]}
        )
        self.assertEqual(
            len([x for x in outgoing_messages["alpha"] if x.status == "in_progress"]), 0
        )
...
class CaptureTest(PostStartupTests):
    ...
    def test_send_snapshot_config_exclude_subdirs_callback(self):
        config = self._get_test_config()
        config.exclude_subdirs_callback = lambda ffs, source_node, target_node: [
            "donotsync"
        ]
        e, outgoing_messages = self.get_engine(
            {
    ...
    def test_pruning_after_capture(self):
        ...
        cfg.decide_snapshots_to_keep = decide
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["b", "a"]}, "beta": {"one": ["b", "a"]}}, config=cfg
        )
        self.assertFalse(outgoing_messages)
        e.incoming_client({"msg": "capture", "ffs": "one"})
        self.assertMsgEqual(
            remove_snapshot_from_message(outgoing_messages[0]),
            {"msg": "capture", "ffs": "one", "to": "alpha"},
        )
        sn = outgoing_messages[0]["snapshot"]
        self.assertTrue(sn in e.model["one"]["alpha"]["upcoming_snapshots"])
        e.incoming_node(
            {"msg": "capture_done", "from": "alpha", "ffs": "one", "snapshot": sn}
        )
        ...
        self.assertMsgEqual(
            outgoing_messages[2],
            {"msg": "remove_snapshot", "to": "alpha", "ffs": "one", "snapshot": "b"},
        )
        self.assertEqual(len(outgoing_messages), 3)
        ...
        self.assertMsgEqual(
            outgoing_messages[0],
            {"msg": "remove_snapshot", "to": "beta", "ffs": "one", "snapshot": "b"},
        )
    ...
    def test_pruning_after_capture_main_first_then_multiple_in_reply_order(self):
        ...
        self.assertMsgEqual(
            remove_snapshot_from_message(outgoing_messages[0]),
            {"msg": "capture", "ffs": "one", "to": "beta"},
        )
        sn = outgoing_messages[0]["snapshot"]
        e.incoming_node(
            {"msg": "capture_done", "from": "beta", "ffs": "one", "snapshot": sn}
        )
        self.assertMsgEqualMinusSnapshot(
            outgoing_messages[1],
            {
        ...
        self.assertMsgEqual(
            outgoing_messages[0],
            {"msg": "remove_snapshot", "to": "gamma", "ffs": "one", "snapshot": "b"},
        )
        self.assertEqual(len(outgoing_messages), 1)
    ...
    def test_pruning_after_capture_always_leaves_at_least_one_snapshot(self):
        ...
        cfg.decide_snapshots_to_keep = decide
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["a"]}, "beta": {"one": ["a"]}}, config=cfg
        )
        self.assertFalse(outgoing_messages)
        e.incoming_client({"msg": "capture", "ffs": "one"})
        self.assertMsgEqual(
        ...
        self.assertMsgEqual(
            outgoing_messages[2],
            {"msg": "remove_snapshot", "to": "alpha", "ffs": "one", "snapshot": "a"},
        )
        ...
        self.assertMsgEqual(
            outgoing_messages[0],
            {"msg": "remove_snapshot", "to": "beta", "ffs": "one", "snapshot": "a"},
        )
        self.assertEqual(len(outgoing_messages), 1)
    ...
    def test_never_prune_snapshots_in_outgoing_que(self):
        ...
        cfg.decide_snapshots_to_keep = decide
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["a"]}, "beta": {"one": ["a"]}, "gamma": {}}, config=cfg
        )
        self.assertFalse(outgoing_messages)
        ...
        e.incoming_client({"msg": "capture", "ffs": "one"})
        self.assertMsgEqual(
            remove_snapshot_from_message(outgoing_messages[1]),
            {"msg": "capture", "ffs": "one", "to": "alpha"},
        )
        ...
        self.assertMsgEqual(
            outgoing_messages[6],
            {"msg": "remove_snapshot", "to": "gamma", "ffs": "one", "snapshot": "a"},
        )
    ...
    def test_capture_with_triggers_matching_syncs(self):
        ...
        e.incoming_client({"msg": "capture", "ffs": "one"})
        self.assertMsgEqual(
            remove_snapshot_from_message(outgoing_messages[3]),
            {"to": "alpha", "msg": "capture", "ffs": "one"},
        )
    ...
    def test_no_capture_during_rename(self):
        e, outgoing_messages = self.get_engine({"beta": {"_one": ["1"]}})
        e.incoming_client({"msg": "rename", "ffs": "one", "new_name": "two"})
    ...
    def test_capture_if_changed(self):
        ...
        self.assertMsgEqual(
            remove_snapshot_from_message(outgoing_messages[0]),
            {"to": "beta", "msg": "capture_if_changed", "ffs": "one"},
        )
        snapshot_name = outgoing_messages[0]["snapshot"]
        outgoing_messages.clear()
        e.incoming_node(
            {
                "msg": "capture_if_changed_done",
                "from": "beta",
                "ffs": "one",
                "snapshot": snapshot_name,
                "changed": False,
    ...
    def test_capture_all_if_changed(self):
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1"], "_two": ["1"]}, "beta": {"_three": []}}
        )
        self.assertEqual(len(outgoing_messages), 0)
        ...
        self.assertMsgEqual(
            remove_snapshot_from_message(outgoing_messages[0]),
            {"msg": "capture_if_changed", "ffs": "one", "to": "alpha"},
        )
        self.assertMsgEqual(
            remove_snapshot_from_message(outgoing_messages[1]),
            {"msg": "capture_if_changed", "ffs": "three", "to": "beta"},
        )
        self.assertMsgEqual(
            remove_snapshot_from_message(outgoing_messages[2]),
            {"msg": "capture_if_changed", "ffs": "two", "to": "alpha"},
        )
...
class RemoveTarget(PostStartupTests):
    def test_basic(self):
        ...
        self.assertMsgEqual(
            outgoing_messages[0], {"msg": "remove", "ffs": "one", "to": "alpha"}
        )
        self.assertEqual(len(outgoing_messages), 1)
        e.incoming_node({"from": "alpha", "msg": "remove_done", "ffs": "one"})
    ...
    def test_remove_asap_triggers_removal_upon_startup(self):
        ...
        self.assertEqual(len(outgoing_messages), 1)
        self.assertMsgEqual(
            outgoing_messages[0], {"msg": "remove", "ffs": "one", "to": "beta"}
        )
    ...
    def test_remove_asap_on_main_triggers_fault(self):
        def inner():
            e, outgoing_messages = self.get_engine(
                {
                    "alpha": {
                        "_one": ["1", ("ffs:main", "on"), ("ffs:remove_asap", "on")]
                    },
    ...
    def test_readding_while_removing_causes_user_error(self):
        ...
        def inner():
            e.incoming_client({"msg": "add_targets", "ffs": "one", "target": ["beta"]})
    ...
    def test_no_syncs_to_currently_removing_target(self):
        ...
        outgoing_messages.clear()
        e.incoming_node(
            {"from": "alpha", "msg": "capture_done", "ffs": "one", "snapshot": snapshot}
        )
        self.assertEqual(len(outgoing_messages), 1)
        self.assertMsgEqualMinusSnapshot(
            outgoing_messages[0],
    ...
    def test_cant_move_during_remove_if_target_is_not_the_one_being_removed(self):
        ...
        def inner():
            e.incoming_client({"msg": "move_main", "ffs": "one", "target": "gamma"})
    ...
    def test_find_node(self):
        ...
        self.assertEqual(len(outgoing_messages), 1)
        self.assertMsgEqual(
            outgoing_messages[0], {"msg": "remove", "ffs": "one", "to": "gamma"}
        )
...
class AddTargetTests(PostStartupTests):
        ...
        e.incoming_node(
            {
                ...
                "from": "beta",
                "ffs": {
                    "one": {"snapshots": ["1", "2"], "properties": {"ffs:main": "on"}}
                },
            }
        )
        e.incoming_node(
            {
                "msg": "ffs_list",
                "from": "alpha",
                "ffs": {"one": {"snapshots": ["1"], "properties": {"ffs:main": "off"}}},
            }
        )
        e.incoming_node({"msg": "ffs_list", "from": "gamma", "ffs": {}})
    ...
    def test_empty_list_raises(self):
        ...
        def inner():
            e.incoming_client({"msg": "add_targets", "ffs": "one", "targets": []})
    ...
    def test_non_target_raises(self):
        ...
        def inner():
            e.incoming_client({"msg": "add_targets", "ffs": "one", "targets": ["shu"]})
    ...
    def test_add_multiple_targets(self):
        ...
        outgoing_messages.clear()
        e.incoming_client(
            {"msg": "add_targets", "ffs": "two", "targets": ["gamma", "alpha"]}
        )
        ...
        e.incoming_node(
            {
                ...
                "ffs": {
                    "one": {
                        "snapshots": ["1", "send-2", "nosend-2", "send-3"],
                        "properties": {"ffs:main": "on", "readonly": "off"},
                    }
                },
            }
    ...
    def test_add_targets_no_snapshots_to_send_captures_after_add_target(self):
        ...
        self.assertEqual(len(fm.outgoing), 1)
        self.assertMsgEqual(
            remove_snapshot_from_message(fm.outgoing[0]),
            {"msg": "capture", "ffs": "one", "to": "beta"},
        )
    ...
    def test_new_multiple_targets_does_not_capture_straight_away(self):
        ...
        self.assertEqual(len(fm.outgoing), 3)  # the list_ffs messages
        fm.outgoing.clear()
        e.incoming_client({"msg": "new", "ffs": "one", "targets": ["beta", "gamma"]})
        self.assertMsgEqual(
            fm.outgoing[0],
            {
                "msg": "new",
    ...
    def test_repeated_target_handled(self):
        ...
        outgoing_messages.clear()
        e.incoming_client(
            {"msg": "add_targets", "ffs": "two", "targets": ["gamma", "gamma"]}
        )
        self.assertEqual(1, len(outgoing_messages))
    ...
    def test_add_target_default_properties(self):
        ...
        e.incoming_node(
            {
                ...
                "ffs": "two",
                "from": "alpha",
                "properties": {"ffs:main": "on", "readonly": "off", "ffs:test": "23"},
            }
        )
        outgoing_messages.clear()
        ...
        self.assertEqual(
            outgoing_messages[0]["properties"],
            {"ffs:main": "off", "readonly": "on", "ffs:test": "23"},
        )
    ...
    def test_add_target_enforced_properties(self):
        ...
        e.incoming_node(
            {
                ...
                "ffs": "two",
                "from": "alpha",
                "properties": {"ffs:main": "on", "readonly": "off", "ffs:test": "24"},
            }
        )
        outgoing_messages.clear()
        e.incoming_client({"msg": "add_targets", "ffs": "two", "targets": ["beta"]})
        ...
        self.assertEqual(
            outgoing_messages[0]["properties"],
            {"ffs:main": "off", "readonly": "on", "ffs:test": "24"},
        )

    def test_add_target_if_parent_is_not_on_target_fails(self):
        e, outgoing_messages = self.get_engine({"alpha": {"_one": ["1"]}, "beta": {}})
...
class MoveTest(PostStartupTests):
    ...
    def test_move_raises_on_moving_to_non_target(self):
        ...
        self.assertEqual(e.model["one"]["beta"]["properties"]["ffs:test"], "2")
    ...
    def test_no_move_during_new(self):
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1"]}, "beta": {"one": ["1"]}, "gamma": {}}
        )
        e.incoming_client({"msg": "new", "ffs": "two", "targets": ["alpha", "beta"]})
    ...
    def test_no_move_during_rename(self):
        e, outgoing_messages = self.get_engine(
            {"alpha": {"one": ["1"]}, "beta": {"_one": ["1"]}, "gamma": {"one": ["1"]}}
        )
        self.assertEqual(0, len(outgoing_messages))
        e.incoming_client({"msg": "rename", "ffs": "one", "new_name": "two"})

        def inner():
            e.incoming_client({"msg": "move_main", "ffs": "one", "target": "alpha"})
    ...
    def test_move_basic(self):
        ...
        self.assertEqual(
            engine.model["one"]["alpha"]["properties"]["ffs:moving_to"], "-"
        )
    ...
    def test_pruning_only_after_move(self):
        ...
        engine, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1"]}, "beta": {"one": ["1"]}, "gamma": {}}, config=cfg
        )
        # flow is as follows
        # 1 - set ffs:moving=target on old main, set read only.
        # 2 - capture on old main
        ...
        self.assertMsgEqual(
            outgoing_messages[6],
            {"msg": "remove_snapshot", "to": "beta", "ffs": "one", "snapshot": "1"},
        )
        self.assertMsgEqual(
            outgoing_messages[7],
            {"msg": "remove_snapshot", "to": "alpha", "ffs": "one", "snapshot": "1"},
        )
...
class SnapshotPruningTests(EngineTests):
    def test_simple(self):
        engine, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1", "2"]}, "beta": {"one": ["1", "2"]}, "gamma": {}}
        )
...
class ZpoolStatusChecks(EngineTests):
    def ge(self):
        e, outgoing_messages = self.get_engine({"alpha": {"_one": ["1", "2"]}})
        errors = []
    ...
    def test_all_ok_no_response(self):
        ...
        self.assertMsgEqual(outgoing[0], {"msg": "zpool_status", "to": "alpha"})
        e.incoming_node(
            {
                "from": "alpha",
                "msg": "zpool_status",
...
""",
        ...
        self.assertEqual(
            len(errors), 5
        )  # for now, the all clear also comes via the error reporting mechanism

    def test_no_repeated_requests_if_outstanding(self):
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1", "2"]}, "beta": {"one": ["1", "2"]}}
        )
        e.do_zpool_status_check()
...
class ChownTests(PostStartupTests):
    def test_chown_standalone(self):
        ...
        e.incoming_client({"msg": "chown_and_chmod", "ffs": "one", "sub_path": "/"})
        self.assertMsgEqual(
            outgoing_messages[0],
            {
                "msg": "chown_and_chmod",
...
class TestStartupTriggeringActions(EngineTests):
    def test_missing_snapshot_triggers_send(self):
        engine, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1", "2"]}, "beta": {"one": ["1"]}, "gamma": {}}
        )
        self.assertEqual(len(outgoing_messages), 1)
        self.assertMsgEqualMinusSnapshot(
            outgoing_messages[0],
    ...
    def test_additional_snapshots_on_target_trigger_snapshot_removal(self):
        engine, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1"]}, "beta": {"one": ["2", "1"]}, "gamma": {}}
        )
        self.assertEqual(len(outgoing_messages), 1)
        self.assertMsgEqual(
            outgoing_messages[0],
            {"msg": "remove_snapshot", "to": "beta", "ffs": "one", "snapshot": "2"},
        )
    ...
    def test_move_interrupted_stage_1(self):
        ...
        self.assertEqual(
            engine.model["one"]["_move_snapshot"], outgoing_messages[0]["snapshot"]
        )
    ...
    def test_move_interrupted_stage_2_after_capture(self):
        engine, outgoing_messages = self.get_engine(
            {
                "alpha": {
                    "_one": ["1", "2", ("readonly", "on"), ("ffs:moving_to", "beta")]
                },
                "beta": {"one": ["1"]},
                "gamma": {},
            }
        ...
        engine, outgoing_messages = self.get_engine(
            {
                ...
                "beta": {"one": ["1", "2", ("ffs:main", "on"), ("readonly", "off")]},
                "gamma": {},
            }
        )
        self.assertMsgEqual(
    ...
    def test_purge_snapshots(self):
        ...
        self.assertMsgEqual(
            outgoing_messages[0],
            {"to": "alpha", "msg": "remove_snapshot", "ffs": "one", "snapshot": "1"},
        )
        self.assertMsgEqual(
            outgoing_messages[1],
            {"to": "beta", "msg": "remove_snapshot", "ffs": "one", "snapshot": "1"},
        )
        self.assertEqual(len(outgoing_messages), 2)
    ...
    def test_snapshot_on_target_too_much(self):
        ...
        self.assertMsgEqual(
            outgoing_messages[1],
            {"to": "beta", "msg": "remove_snapshot", "ffs": "one", "snapshot": "0"},
        )
        self.assertMsgEqual(
            outgoing_messages[2],
            {"to": "beta", "msg": "remove_snapshot", "ffs": "one", "snapshot": "1"},
    ...
    def test_send_only_some(self):
        ...
        self.assertMsgEqual(
            outgoing_messages[1],
            {"to": "alpha", "msg": "remove_snapshot", "ffs": "one", "snapshot": "1b"},
        )
        self.assertMsgEqual(
            outgoing_messages[2],
            {"to": "beta", "msg": "remove_snapshot", "ffs": "one", "snapshot": "-1"},
    ...
    def test_capture_if_no_snapshots_to_send_but_replicates_decide_on_snapshots(self):
        cfg = self._get_test_config()
        cfg.decide_snapshots_to_send = lambda ffs, snapshots: [
            x for x in snapshots if x.startswith("ffs-")
        ]
        engine, outgoing_messages = self.get_engine(
            {
                # thes that we stick to this order. Nodes return snapshots in
    ...
    def test_prio_ordinging_during_send_missing(self):
        ...
        for i in range(1, 100):
            beta_def["_A%i" % i] = ["1", "2"]
            alpha_def["A%i" % i] = ["1", "2"]
        e, outgoing_messages = self.get_engine({"beta": beta_def, "alpha": alpha_def})
        self.assertMsgEqualMinusSnapshot(
            outgoing_messages[0],
            {
                "msg": "send_snapshot",
...
class CrossTalkTest(EngineTests):
    def test_capture_during_move(self):
        ...
        # veryify move worked
        self.assertEqual(e.model["one"]["_main"], "beta")
        self.assertEqual(e.model["one"]["alpha"]["properties"]["ffs:main"], "off")
        self.assertEqual(e.model["one"]["beta"]["properties"]["ffs:main"], "on")
        self.assertEqual(e.model["one"]["alpha"]["properties"]["readonly"], "on")
        self.assertEqual(e.model["one"]["beta"]["properties"]["readonly"], "off")
        self.assertEqual(e.model["one"]["alpha"]["properties"]["ffs:moving_to"], "-")
...
class MockEngine:
    def __init__(self):
        self.node_messages = []
        self.config = default_config.CheckedConfig(default_config.DefaultConfig())
...
class RenameTests(PostStartupTests):
    def test_rename_non_replicated(self):
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1"]}, "beta": {}, "gamma": {}}
        )
        self.assertEqual(0, len(outgoing_messages))
        e.incoming_client(
            {"msg": "rename", "to": "alpha", "ffs": "one", "new_name": "two"}
        )
        self.assertMsgEqual(
            outgoing_messages[0],
            {"msg": "rename", "to": "alpha", "ffs": "one", "new_name": "two"},
        ...
        self.assertEqual(1, len(outgoing_messages))
        e.incoming_node(
            {"msg": "rename_done", "ffs": "one", "from": "alpha", "new_name": "two"}
        )
    ...
    def test_rename_raises_duplicate_name(self):
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["1"], "_two": ["1"]}, "beta": {}, "gamma": {}}
        )
        self.assertEqual(0, len(outgoing_messages))
    ...
    def test_rename_replicated(self):
        ...
        self.assertMsgEqual(
            outgoing_messages[1],
            {"msg": "rename", "to": "beta", "ffs": "one", "new_name": "two"},
        )
        self.assertMsgEqual(
            outgoing_messages[2],
            {"msg": "rename", "to": "gamma", "ffs": "one", "new_name": "two"},
        )
        ...
        e.incoming_node(
            {"msg": "rename_done", "ffs": "one", "from": "gamma", "new_name": "two"}
        )
        self.assertFalse("gamma" in e.model["one"])
        ...
        e.incoming_node(
            {"msg": "rename_done", "ffs": "one", "from": "beta", "new_name": "two"}
        )
    ...
    def test_rename_replicated_interrupted_main_was_renamed(self):
        ...
        self.assertTrue("two" in e.model)
        self.assertEqual(
            e.model["two"]["beta"]["properties"].get("ffs:renamed_from", "-"), "-"
        )
        self.assertEqual(e.model["two"]["_main"], "beta")
    ...
    def test_rename_requires_ffs(self):
        ...
        def inner():
            e.incoming_client({"msg": "rename", "new_name": "two"})
    ...
    def test_rename_requires_new_name(self):
        ...
        def inner():
            e.incoming_client({"msg": "rename", "ffs": "one"})
    ...
    def test_rename_invalid_ffs(self):
        ...
        def inner():
            e.incoming_client({"msg": "rename", "ffs": "two", "new_name": "three"})
    ...
    def test_duplicate_rename_raises(self):
        ...
        def inner():
            e.incoming_client({"msg": "rename", "ffs": "one", "new_name": "three"})
    ...
    def test_rename_raises_on_unequal_parents(self):
        ...
        def inner():
            e.incoming_client({"msg": "rename", "ffs": "one/a", "new_name": "two/a"})
...
class ChmodTests(PostStartupTests):
    def test_basic(self):
        e, outgoing_messages = self.get_engine({"beta": {"_one": ["1"]}})
        r = e.incoming_client(
            {"msg": "chown_and_chmod", "ffs": "one", "sub_path": "/code"}
        )
    ...
    def test_raises_no_ffs(self):
        ...
        def inner():
            e.incoming_client({"msg": "chown_and_chmod", "sub_path": "code"})
    ...
    def test_raises_invalid_ffs(self):
        ...
        def inner():
            e.incoming_client(
                {"msg": "chown_and_chmod", "ffs": "two", "sub_path": "code"}
            )
    ...
    def test_raises_no_sub_path(self):
        ...
        def inner():
            e.incoming_client({"msg": "chown_and_chmod", "ffs": "one"})
...
class TimeBasedSnapshotTests(PostStartupTests):
    def test_setting_interval_on(self):
        e, outgoing_messages = self.get_engine(
            {"beta": {"_one": ["1"]}, "alpha": {"one": ["1"]}}
        )
        e.incoming_client(
            {"msg": "set_snapshot_interval", "ffs": "one", "interval": 34}
        )
        self.assertMsgEqual(
            outgoing_messages[0],
            {
    ...
    def test_setting_interval_no_ffs(self):
        ...
        def inner():
            e.incoming_client({"msg": "set_snapshot_interval", "interval": 34})
    ...
    def test_setting_interval_invalid_ffs(self):
        ...
        def inner():
            e.incoming_client(
                {"msg": "set_snapshot_interval", "ffs": "four", "interval": 34}
            )
    ...
    def test_setting_interval_no_interval(self):
        ...
        def inner():
            e.incoming_client({"msg": "set_snapshot_interval", "ffs": "one"})
        ...
        e.incoming_node(
            {
                ...
                "from": "beta",
                "ffs": "eight",
                "snapshot": e.model["eight"]["beta"]["upcoming_snapshots"][0],
                "changed": True,
            }
        )
        e.incoming_node(
            {
                ...
                "from": "beta",
                "ffs": "nine",
                "snapshot": e.model["nine"]["beta"]["upcoming_snapshots"][0],
                "changed": False,
            }
        )
    ...
    def test_no_interval_if_faulted(self):
        ...
        e, outgoing_messages = self.get_engine(
            {"beta": {"_five": [name_snapshot(-161), ("ffs:snapshot_interval", 1)]}}
        )
        e.one_minute_passed()
        # time has passed
        self.assertTrue(e.model["five"]["beta"]["upcoming_snapshots"])
    ...
    def test_keep_snapshots_time_policy(self):
        ...
        actual = default_config.keep_snapshots_time_policy(
            sn,
            quarters=4,
            hours=24,
            days=7,
            ...
            months=12,
            years=15,
            allow_one_snapshot_to_fill_multiple_intervals=True,
            now=now.timestamp(),
        )
        self.assertEqual(actual, should_keep)
        actual2 = default_config.keep_snapshots_time_policy(
            sn,
            quarters=4,
            hours=24,
            days=7,
        ...
        self.assertTrue(
            len(actual2) > len(actual)
        )  # actual2 must have *more* snapshots
...
class ClientFacingTests(PostStartupTests):
    ...
    def test_list_ffs_full(self):
        e, outgoing_messages = self.get_engine(
            {"beta": {"_one": ["1", ("ffs:shu", "123")]}, "alpha": {"one": ["1"]}}
        )
        l = e.incoming_client({"msg": "list_ffs", "full": True})
        self.assertEqual(
            l,
            {
                "one": {
    ...
    def test_list_ffs_full_during_new(self):
        e, outgoing_messages = self.get_engine(
            {"beta": {"_one": ["1", ("ffs:shu", "123")]}, "alpha": {"one": ["1"]}}
        )
        e.incoming_client({"msg": "new", "ffs": "two", "targets": ["alpha"]})
        ...
        self.assertEqual(
            l,
            {
                ...
                "two": {"targets": ["alpha"], "properties": {"alpha": {}}},
            },
        )
    ...
    def test_client_list_ffs_in_case_of_add_target(self):
        e, outgoing_messages = self.get_engine({"beta": {"_one": ["1"]}, "alpha": {}})
        l = e.incoming_client({"msg": "list_ffs"})
...
class FailureTests(PostStartupTests):
    ...
    def test_snapshot_removal_fails_due_to_clones(self):
        ...
        e, outgoing_messages = self.get_engine(
            {"alpha": {"_one": ["0"]}, "beta": {"one": ["-1", "0"]}, "gamma": {}},
            config=cfg,
        )
        self.assertEqual(len(outgoing_messages), 1)
        informs.clear()
...
class ReadOnlyHostTests(PostStartupTests):
    def ge(self):
        engine, outgoing_messages = self.get_engine(
            ...
            additional_node_config={"alpha": {"readonly_node": True}},
        )
    ...
    def test_new_readonly_node_raises(self):
        ...
        def inner():
            e.incoming_client(
                {"msg": "new", "ffs": "three", "targets": ["alpha", "beta"]}
            )

        self.assertRaises(engine.NodeIsReadonly, inner)
        e.incoming_client({"msg": "new", "ffs": "three", "targets": ["beta"]})
        self.assertEqual(len(outgoing_messages), 1)
    ...
    def test_set_interval_read_only_node_raises(self):
        ...
        def inner():
            e.incoming_client(
                {"msg": "set_snapshot_interval", "ffs": "one", "interval": 500}
            )

        self.assertRaises(engine.NodeIsReadonly, inner)
        e.incoming_client(
            {"msg": "set_snapshot_interval", "ffs": "two", "interval": 500}
        )
        self.assertEqual(len(outgoing_messages), 1)
        e.incoming_client(
            {"msg": "set_snapshot_interval", "ffs": "four", "interval": 500}
        )
        self.assertEqual(len(outgoing_messages), 2)
    ...
    def test_chown_read_only_main_raises(self):
        ...
        self.assertRaises(engine.NodeIsReadonly, inner)
        e.incoming_client({"msg": "chown_and_chmod", "ffs": "two", "sub_path": "/"})
        self.assertEqual(len(outgoing_messages), 1)
    ...
    def test_set_priority_readonly_node_raises(self):
        ...
        def inner():
            e.incoming_client({"msg": "set_priority", "ffs": "one", "priority": 500})

        self.assertRaises(engine.NodeIsReadonly, inner)
        e.incoming_client({"msg": "set_priority", "ffs": "two", "priority": 500})
        self.assertEqual(len(outgoing_messages), 1)
        e.incoming_client({"msg": "set_priority", "ffs": "four", "priority": 500})
        self.assertEqual(len(outgoing_messages), 2)
    ...
    def test_no_moving_readonly_node(self):
        ...
        def inner():
            e.incoming_client({"msg": "move_main", "ffs": "two", "target": "alpha"})
...
class PriorityTests(PostStartupTests):
    ...
    def test_client_set_prio_not_specified(self):
        ...
        def inner():
            e.incoming_client({"msg": "set_priority", "ffs": "one", "interval": 34})
    ...
    def test_client_set_prio(self):
        ...
        e.incoming_client({"msg": "set_priority", "ffs": "one", "priority": 34})
        self.assertMsgEqual(
            outgoing_messages[0],
            {
                "msg": "set_properties",
    ...
    def test_prio_reorder(self):
        ...
        msgs = [
            ssh_message_que.MessageInProgress(
                "node1",
                {},
                {"msg": "remove_snapshot", "priority": 1000, "snapshot": "b"},
            ),
            ssh_message_que.MessageInProgress(
                "node1",
                {},
                {"msg": "remove_snapshot", "priority": 900, "snapshot": "a"},
            ),
            ssh_message_que.MessageInProgress(
                "node1", {}, {"msg": "capture", "ffs": "b"}
            ),
            ssh_message_que.MessageInProgress("node1", {}, {"msg": "new", "ffs": "a"}),
            ssh_message_que.MessageInProgress(
                "node1", {}, {"msg": "send_snapshot", "ffs": "b", "snapshot": "a"}
            ),
            ssh_message_que.MessageInProgress(
                "node1",
                {},
