...
class EngineTests(unittest.TestCase):
    def assertMsgEqualMinusSnapshot(self, actual, supposed):
        msg = actual.copy()
        for k in [
                'target_ssh_cmd', 'target_user', 'target_ffs', 'target_node',
                'target_storage_prefix', 'excluded_subdirs'
        ]:
...
class StartupTests(EngineTests):
    ...
    def test_startup_sends_deployment_followed_by_list_ffs(self):
        def strip_node_zip(msg):
            if 'node.zip' in msg:
                import base64
                base64.decodebytes(
                    msg['node.zip'].encode('utf-8'))  # make sure it decodes
                msg['node.zip'] = '...base64...'
            return msg
    ...
    def test_restoring_main_from_ro(self):
        ...
        self.assertEqual(e.model['one']['beta']['properties']['readonly'],
                         'off')
        # no ffs info -> nothing happens
        ...
        self.assertEqual(e.model['one']['beta']['properties']['readonly'],
                         'off')
        self.assertEqual(e.model['one']['beta']['properties']['ffs:main'],
                         'on')
    ...
    def test_enforced_properties_set_if_unset(self):
        e, outgoing_messages = self.ge(
            self._get_cfg_enforced_properties({
                'ffs:test': 'shu'
            }))
        self.assertFalse(e.startup_done)
        e.incoming_client({
...
class NewTests(PostStartupTests):
    ...
    def test_new_single_node(self):
        ...
        self.assertEqual(e.model['two']['alpha']['properties']['ffs:main'],
                         'on')
        self.assertEqual(e.model['two']['alpha']['properties']['readonly'],
                         'off')
    ...
    def test_new_multi_nodes(self):
        ...
        self.assertEqual(e.model['two']['beta']['properties']['ffs:main'],
                         'on')
        self.assertEqual(e.model['two']['beta']['properties']['readonly'],
                         'off')
    ...
    def test_nested_creation_delayed_until_parent_has_been_created(self):
        ...
        self.assertEqual(
            len([
                x for x in outgoing_messages['alpha']
    ...
    def test_new_parent_issue(self):
        ...
        self.assertEqual(
            len([
                x for x in outgoing_messages['beta']
                if x.status == 'in_progress'
            ]), 1)
...
class CaptureTest(PostStartupTests):
    ...
    def test_pruning_after_capture_main_first_then_multiple_in_reply_order(
            self):
        cfg = self._get_test_config()
...
class RemoveTarget(PostStartupTests):
    ...
    def test_remove_asap_on_main_triggers_fault(self):
        def inner():
            e, outgoing_messages = self.get_engine({
                'alpha': {
                    '_one':
                    ['1', ('ffs:main', 'on'), ('ffs:remove_asap', 'on')]
                },
                'beta': {
                    'one': ['1', ('ffs:main', 'off'), (
                        'readonly',
    ...
    def test_cant_move_during_remove_if_target_is_not_the_one_being_removed(
            self):
        e, outgoing_messages = self.get_engine({
            'alpha': {
                '_one': [
...
class MoveTest(PostStartupTests):
    def test_move_raises_on_moving_to_main(self):
        ...
        self.assertEqual(engine.model['one']['beta']['properties']['ffs:test'],
                         '2')
    ...
    def test_move_basic(self):
        ...
        self.assertEqual(
            engine.model['one']['alpha']['properties']['ffs:main'], 'off')
        self.assertEqual(engine.model['one']['beta']['properties']['ffs:main'],
                         'on')
        self.assertEqual(
            engine.model['one']['alpha']['properties']['readonly'], 'on')
        self.assertEqual(engine.model['one']['beta']['properties']['readonly'],
                         'off')
        self.assertEqual(
...
'''
        ...
        self.assertEqual(len(errors),
                         2)  # DEGRADING something else is another error
...
'''
        ...
        self.assertEqual(len(errors),
                         3)  # failing is most certainly an error again
...
class TestStartupTriggeringActions(EngineTests):
    ...
    def test_move_interrupted_stage_1(self):
        engine, outgoing_messages = self.get_engine({
            'alpha': {
                '_one': ['1', ('readonly', 'on'), ('ffs:moving_to', 'beta')]
            },
            'beta': {
                'one': [
                    '1',
        ...
        self.assertEqual(engine.model['one']['_move_snapshot'],
                         outgoing_messages[0]['snapshot'])

    def test_move_interrupted_stage_2_after_capture(self):
        engine, outgoing_messages = self.get_engine({
            'alpha': {
                '_one':
                ['1', '2', ('readonly', 'on'), ('ffs:moving_to', 'beta')]
            },
            'beta': {
                'one': ['1']
            },
    ...
    def test_move_interrupted_stage_5_after_set_main(self):
        engine, outgoing_messages = self.get_engine({
            ...
            'beta': {
                'one': ['1', '2', ('ffs:main', 'on'), ('readonly', 'off')]
            },
            'gamma': {},
        })
        self.assertMsgEqual(
    ...
    def test_capture_if_no_snapshots_to_send_but_replicates_decide_on_snapshots(
            self):
        cfg = self._get_test_config()
...
class CrossTalkTest(EngineTests):
    def test_capture_during_move(self):
        ...
        # veryify move worked
        self.assertEqual(e.model['one']['_main'], 'beta')
        self.assertEqual(e.model['one']['alpha']['properties']['ffs:main'],
                         'off')
        self.assertEqual(e.model['one']['beta']['properties']['ffs:main'],
                         'on')
        self.assertEqual(e.model['one']['alpha']['properties']['readonly'],
                         'on')
        self.assertEqual(e.model['one']['beta']['properties']['readonly'],
                         'off')
        self.assertEqual(
            e.model['one']['alpha']['properties']['ffs:moving_to'], '-')
...
class OutgoingMessageTests(unittest.TestCase):
    ...
    def test_max_one_send_snapshot_for_one_ffs_irregardless_of_max_concurrent(
            self):
        m = {'msg': 'send_snapshot', 'ffs': 'one'}
        om = OutgoingMessageForTesting()
    ...
    def test_rename_and_having_to_restore_main_readonly_faults_engine(self):
        def inner():
            e, outgoing_messages = self.get_engine({
                ...
                'beta': {
                    'two': ['1', ('readonly', 'off'), ('ffs:main', 'off')]
                },
                'gamma': {
                    'one': ['1']
                },
...
class TimeBasedSnapshotTests(PostStartupTests):
    ...
    def test_invalid_interval_during_startup_faults(self):
        def inner():
            e, outgoing_messages = self.get_engine({
                'beta': {
                    '_one': ['1', ('ffs:snapshot_interval', 'shu')]
                },
            })
    ...
    def test_negative_interval_during_startup(self):
        def inner():
            e, outgoing_messages = self.get_engine({
                'beta': {
                    '_one': ['1', ('ffs:snapshot_interval', '-1')]
                },
            })
    ...
    def test_auto_snapshot(self):
        ...
        e, outgoing_messages = self.get_engine({
            'beta': {
                ...
                '_three':
                ['ffs-shubidudh', ('ffs:snapshot_interval', 15 * 60)],
                '_four':
                [name_snapshot(0), ('ffs:snapshot_interval', 15 * 60)],
                '_five':
                [name_snapshot(-161), ('ffs:snapshot_interval', 1 * 60)],
                '_six': [
                    name_snapshot(-161), ('ffs:snapshot_interval', 1 * 60)
                ],
                '_seven': [
        ...
        e.incoming_node({
            'msg':
            'capture_if_changed_done',
            'from':
            'beta',
            'ffs':
            'eight',
            'snapshot':
            e.model['eight']['beta']['upcoming_snapshots'][0],
            'changed':
            True
        })
        e.incoming_node({
            ...
            'nine',
            'snapshot':
            e.model['nine']['beta']['upcoming_snapshots'][0],
            'changed':
            False
        })
    ...
    def test_main_has_no_interval_others_do(self):
        def inner():
            e, outgoing_messages = self.get_engine({
                ...
                'alpha': {
                    'one': ['1', ('ffs:snapshot_interval', '50')]
                },
            })
    ...
    def test_conflicting_intervals_between_main_and_target(self):
        e, outgoing_messages = self.get_engine({
            'beta': {
                '_one': ['1', ('ffs:snapshot_interval', '100')]
            },
            'alpha': {
                'one': ['1', ('ffs:snapshot_interval', '50')]
            },
        ...
        self.assertMsgEqual(
            outgoing_messages[0], {
                ...
                'ffs': 'one',
                'properties': {
                    'ffs:snapshot_interval': '100'
                },
                'to': 'alpha'
            })

    def test_new_not_send_or_finished_and_one_minute_passed_does_not_raise(
            self):
    ...
    def test_keep_snapshots_time_policy(self):
        ...
        actual2 = default_config.keep_snapshots_time_policy(
            ...
            now=now.timestamp())
        self.assertTrue(
            len(actual2) > len(actual))  # actual2 must have *more* snapshots
...
class PriorityTests(PostStartupTests):
    ...
    def test_prio_reorder(self):
        ...
        msgs = [
            ssh_message_que.MessageInProgress('node1', {}, {
                'msg': 'remove_snapshot',
                'priority': 1000,
                'snapshot': 'b'
            }),
