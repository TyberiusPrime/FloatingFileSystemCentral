...
class Config(DefaultConfig):
    ...
    def get_logging(self):
        ...
        if self._get_logging_count > 1:
            raise ValueError(
                "Repeated calls to get_logging - are you accessing outside of CheckedConfig?"
            )
        ...
        debug_logger = logging.handlers.RotatingFileHandler(
            os.path.join(os.path.dirname(__file__), "client_test_debug.log"),
            mode="w",
        ...
        error_logger = logging.handlers.RotatingFileHandler(
            os.path.join(os.path.dirname(__file__), "client_test_error.log"),
            mode="w",
            maxBytes=10 * 1024 * 1024,
            backupCount=1,
            encoding=None,
